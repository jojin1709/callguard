...
const blocklistSchema = z.object({
  ...
  reason: z.string().max(200).optional().transform((v) => v ? stripHtml(v.trim()) : undefined),
});
...
export async function POST(req: Request) {
  ...
  const rl = rateLimit(`blocklist-${ip}`, 30, 60000);
  if (!rl.allowed) {
  ...
  const entry = await prisma.blocklistEntry.upsert({
    ...
    create: { userId, phoneNumberId: phoneNumber.id, reason: reason || null },
...
export async function DELETE(req: Request) {
  const ip = getClientIp(req);
  const rl = rateLimit(`blocklist-del-${ip}`, 30, 60000);
  if (!rl.allowed) {
    return NextResponse.json({ error: "Rate limit exceeded." }, { status: 429 });
  }
  ...
  await prisma.blocklistEntry.deleteMany({ where: { id, userId } });
